...
// @description              Add AI answers to Google Search (powered by Google Gemma + the latest LLMs!)
// @description:af           Voeg AI-antwoorde by Google Search (aangedryf deur Google Gemma + the latest LLMs!)
// @description:am           የ Google Search ውስጥ AI መልቀቅን አድርግ፣ (Google Gemma + the latest LLMs በመሣሪያዎቹ ውስጥ!)
// @description:ar           يضيف إجابات AI إلى Google Search (مدعوم بواسطة Google Gemma + the latest LLMs!)
// @description:as           Google Search-লৈ AI উত্তৰ যোগ দিয়ে (Google Gemma + the latest LLMs দ্বাৰা পাওৱা হৈছে!)
// @description:az           Google Search-ya AI cavablarını əlavə edir (Google Gemma + the latest LLMs tərəfindən dəstəklənir!)
// @description:be           Дадае ІА адказы на Google Search (падтрымліваецца Google Gemma + the latest LLMs!)
// @description:bg           Добавя ИИ отговори в Google Search (поддържан от Google Gemma + the latest LLMs!)
// @description:bn           Google Search-ত AI উত্তর যোগ করে (Google Gemma + the latest LLMs দ্বারা প্রচালিত!)
// @description:bs           Dodaje AI odgovore na Google Search (pokreće Google Gemma + the latest LLMs!)
// @description:ca           Afegeix respostes d'IA a Google Search (impulsat per Google Gemma + the latest LLMs!)
// @description:ceb          Nagdugang ug mga tubag AI ngadto sa Google Search (gipadagan sa Google Gemma + the latest LLMs!)
// @description:co           Aggiunge risposte AI a Google Search (supportate da Google Gemma + the latest LLMs!)
// @description:cs           Přidává AI odpovědi do Google Search (poháněno Google Gemma + the latest LLMs!)
// @description:cy           Ychwanegu atebion AI i Google Search (a yrrir gan Google Gemma + the latest LLMs!)
// @description:da           Tilføjer AI-svar til Google Search (drevet af Google Gemma + the latest LLMs!)
// @description:de           Fügt AI-Antworten zu Google Search hinzu (betrieben von Google Gemma + den neuesten LLMs!)
// @description:el           Προσθέτει απαντήσεις AI στο Google Search (τροφοδοτούμενο από Google Gemma + τα τελευταία LLMs!)
// @description:en           Add AI answers to Google Search (powered by Google Gemma + the latest LLMs!)
// @description:eo           Aldonas AI-respondojn al Google Search (ebligita de Google Gemma + la plej novaj LLMs!)
// @description:es           Añade respuestas de IA a Google Search (impulsado por Google Gemma + los últimos LLMs!)
// @description:et           Lisab AI-vastused Google Search'le (juhitud Google Gemma + uusimate LLM-idega!)
// @description:eu           Gehitu IA erantzunak Google Search-n (Google Gemma + azken LLMs-ek bultzatuta!)
// @description:fa           پاسخهای هوشمصنوعی به Google Search اضافه میشود (توسط Google Gemma + جدیدترین LLMs پشتیبانی میشود!)
// @description:fi           Lisää tekoälyvastauksia Google Search:hun (ohjattu Google Gemma + uusimmilla LLM:illä!)
// @description:fil          Nagdaragdag ng mga sagot ng AI sa Google Search (pinapagana ng Google Gemma + ang pinakabagong LLMs!)
// @description:fo           Bætir AI svar við Google Search (drifin af Google Gemma + nýjustu LLM!)
// @description:fr           Ajoute des réponses IA à Google Search (propulsé par Google Gemma + les derniers LLMs !)
// @description:fr-CA        Ajoute des réponses IA à Google Search (propulsé par Google Gemma + les derniers LLMs !)
// @description:fy           Foeget AI-antwurden ta oan Google Search (dreaun troch Google Gemma + de lêste LLMs!)
// @description:ga           Cuirtear freagraí AI le Google Search (dírítear ag Google Gemma + na LLManna is déanaí!)
// @description:gd           Cur freagairtichean AI ris an Google Search (air a thug seachad le Google Gemma + na LLMan as ùire!)
// @description:gl           Engade respostas de IA a Google Search (impulsado por Google Gemma + os últimos LLMs!)
// @description:gu           Google Search માટે AI જવાબો ઉમેરે છે (Google Gemma + નવીનતમ LLMs દ્વારા પોવરેડ!)
// @description:ha           Ƙaddara takardun AI zu Google Search (da aka fi Google Gemma + sabbin LLMs!)
// @description:haw          Hoʻohui aku i nā hoʻopiʻi AI iā Google Search (hoʻohui ʻia e Google Gemma + nā LLM hou loa!)
// @description:he           מוסיף תשובות AI ל-Google Search (מופעל על ידי Google Gemma + ה-LLMs העדכניים ביותר!)
// @description:hi           Google Search में AI उत्तर जोड़ता है (Google Gemma + नवीनतम LLMs द्वारा संचालित!)
// @description:hmn          Ntxig AI nruab nruab rau Google Search (pab cuam Google Gemma + LLMs tshiab!)
// @description:hr           Dodaje AI odgovore na Google Search (pokreće Google Gemma + najnoviji LLM-ovi!)
// @description:ht           Ajoute repons AI nan Google Search (pòte pa Google Gemma + dènye LLMs yo!)
// @description:hu           AI válaszokat ad hozzá a Google Search-hoz (Google Gemma + a legújabb LLM-ek által hajtva!)
// @description:hy           Ավելացնում է AI պատասխաններ Google Search-ում (աջակցված է Google Gemma + վերջին LLM-ներով!)
// @description:ia           Adde responas AI a Google Search (propulsate per Google Gemma + le ultime LLMs!)
// @description:id           Menambahkan jawaban AI ke Google Search (didukung oleh Google Gemma + LLM terbaru!)
// @description:ig           Tinye ihe ndekọ AI n'ụzọ ọgụgụ Google Search (n'efu na Google Gemma + LLMs ọhụrụ!)
// @description:ii           Google Search ᐸᔦᒪᔪᐃᓃᑦ AI ᓇᑕᐅᒪᐃᑦᓯ (Google Gemma + ᓂᑕᔪᑦᓯᐏᑦᑕᒥᔭ!)
// @description:is           Bætir AI svar við Google Search (keyrir á Google Gemma + nýjustu LLM!)
// @description:it           Aggiunge risposte AI a Google Search (alimentato da Google Gemma + gli ultimi LLMs!)
// @description:iu           Google Search ᑲᑎᒪᔪᖅᑐᖅᑐᐃᓐᓇᓂᒃ AI ᑎᑎᕋᖃᕐᓯᒪᓂᖏᓐ (Google Gemma + ᑐᑭᒧᑦᑖᑦ!)
// @description:ja           Google Search に AI 回答を追加します (Google Gemma + 最新のLLMsで動作！)
// @description:jv           Nambéhi pirangga AI nganti Google Search (diduweni déning Google Gemma + LLM paling anyar!)
// @description:ka           ამატებს AI პასუხებს Google Search-ს (იმართება Google Gemma + უახლესი LLM-ებით!)
// @description:kk           Google Search-ға AI жауаптарын қосады (Google Gemma + соңғы LLM арқылы жұмыс істейді!)
// @description:kl           Google Search-mi AI-t Kalaallit Nunaanni iluani (Google Gemma + -nip ilaanni!)
// @description:km           បន្ថែមចម្លើយ AI ទៅ Google Search (ដំណើរការដោយ Google Gemma + LLMs ចុងក្រោយបំផុត!)
// @description:kn           Google Search ಗೆ AI ಉತ್ತರಗಳನ್ನು ಸೇರಿಸುತ್ತದೆ (Google Gemma + ಇತ್ತೀಚಿನ LLMs ನಿಂದ ನಡೆಸಲ್ಪಡುತ್ತಿದೆ!)
// @description:ko           Google Search에 AI 답변을 추가합니다 (Google Gemma + 최신 LLMs 제공!)
// @description:ku           Bersivên AI-ê li Google Search zêde dike (ji hêla Google Gemma + LLM-ên herî dawî ve hatî hêzdar kirin!)
// @description:ky           Google Search'го AI жоопторун кошот (Google Gemma + акыркы LLMs тарабынан иштейт!)
// @description:la           Addit AI responsa Google Search (powered per Google Gemma + recentissimis LLMs!)
// @description:lb           Füügt AI Äntwerten op Google Search (ugedriwwen duerch Google Gemma + déi lescht LLMs!)
// @description:lg           Yambula emisomo ey'ensobi ku Google Search (enkuuma Google Gemma + LLM ezisinga!)
// @description:ln           Ebakisi biyano ya AI na Google Search (ezali na nguya ya Google Gemma + LLMs ya sika!)
// @description:lo           ເພີ່ມຄໍາຕອບ AI ໃຫ້ກັບ Google Search (ຂັບເຄື່ອນໂດຍ Google Gemma + LLMs ຫຼ້າສຸດ!)
// @description:lt           Prideda AI atsakymus į „Google Search“ (maitina Google Gemma + naujausi LLM!)
// @description:lv           Pievieno AI atbildes Google Search (darbina Google Gemma + jaunākie LLM!)
// @description:mg           Manampy valiny AI amin'ny Google Search (nampiasain'ny Google Gemma + LLM farany!)
// @description:mi           Ka taapirihia nga whakautu AI ki a Google Search (whakamahia e Google Gemma + ngā LLM hou!)
// @description:mk           Додава одговори со вештачка интелигенција на Google Search (напојувано од Google Gemma + најновите LLM!)
// @description:ml           Google Search-യിലേക്ക് AI ഉത്തരങ്ങൾ ചേർക്കുന്നു (Google Gemma + ഏറ്റവും പുതിയ LLMs നൽകുന്നതാണ്!)
// @description:mn           Google Search-д AI хариултуудыг нэмдэг (Google Gemma + хамгийн сүүлийн LLM-ээр ажилладаг!)
// @description:mr           Google Search ला AI उत्तरे जोडते (Google Gemma + नवीनतम LLMs द्वारे समर्थित!)
// @description:ms           Menambahkan jawapan AI pada Google Search (dikuasakan oleh Google Gemma + LLM terkini!)
// @description:mt           Iżżid it-tweġibiet AI għal Google Search (mħaddma minn Google Gemma + l-aħħar LLMs!)
// @description:my           Google Search (Google Gemma + နောက်ဆုံးထွက် LLMs ဖြင့် စွမ်းဆောင်ထားသည့်) တွင် AI အဖြေများကို ပေါင်းထည့်သည်
// @description:na           Aeta AI teroma i Google Search (ira Google Gemma + LLM rodu!)
// @description:nb           Legger til AI-svar på Google Search (drevet av Google Gemma + de nyeste LLM-ene!)
// @description:nd           Iyatholakala amaswelelo e-AI kuGoogle Search (kuyatholakala ngokulawula uGoogle Gemma + LLM zakamuva!)
// @description:ne           Google Search मा AI जवाफहरू थप्छ (Google Gemma + नवीनतम LLMs द्वारा संचालित!)
// @description:ng           Ondjova mbelelo dha AI moGoogle Search (uumbuli nguGoogle Gemma + LLM dha ungheri!)
// @description:nl           Voegt AI-antwoorden toe aan Google Search (mogelijk gemaakt door Google Gemma + de nieuwste LLMs!)
// @description:nn           Legg til AI-svar på Google Search (drevet av Google Gemma + dei nyaste LLM-ane!)
// @description:no           Legger til AI-svar til Google Search (drevet av Google Gemma + de nyeste LLM-ene!)
// @description:nso          Ya go etela ditshenyegi tsa AI mo Google Search (e dirwang ke Google Gemma + LLM tša morago!)
// @description:ny           Imawonjezera mayankho a AI ku Google Search (yoyendetsedwa ndi Google Gemma + ma LLM aposachedwa!)
// @description:oc           Ajusta de respòstas d'IA a Google Search (amb Google Gemma + los darrièrs LLMs!)
// @description:om           Deebii AI Google Search (Google Gemma + LLM haaraa'n kan hojjetu!) irratti dabalata.
// @description:or           Google Search କୁ AI ଉତ୍ତର ଯୋଗ କରେ (Google Gemma + ସଦ୍ୟତମ LLMs ଦ୍ୱାରା ଚାଳିତ!)
// @description:pa           Google Search (Google Gemma + ਨਵੀਨਤਮ LLMs ਦੁਆਰਾ ਸੰਚਾਲਿਤ!) ਵਿੱਚ AI ਜਵਾਬ ਸ਼ਾਮਲ ਕਰਦਾ ਹੈ
// @description:pl           Dodaje odpowiedzi AI do Google Search (obsługiwane przez Google Gemma + najnowsze LLM-y!)
// @description:ps           Google Search ته د AI ځوابونه اضافه کوي (د Google Gemma + وروستي LLMs لخوا پرمخ وړل کیږي!)
// @description:pt           Adiciona respostas de IA ao Google Search (desenvolvido por Google Gemma + os LLMs mais recentes!)
// @description:pt-BR        Adiciona respostas de IA ao Google Search (desenvolvido por Google Gemma + os LLMs mais recentes!)
// @description:qu           Google Search (Google Gemma + LLM maymanta!) nisqaman AI kutichiykunata yapan.
// @description:rm           Agiuntescha respostas d'IA a Google Search (propulsà da Google Gemma + ils ultims LLMs!)
// @description:rn           Abafasha inyandiko z'IA ku Google Search (yashyizweho na Google Gemma + LLM vuba!)
// @description:ro           Adaugă răspunsuri AI la Google Search (alimentat de Google Gemma + cele mai recente LLM-uri!)
// @description:ru           Добавляет ответы ИИ в Google Search (на базе Google Gemma + последних LLMs!)
// @description:rw           Ongeraho ibisubizo bya AI kuri Google Search (ikoreshwa na Google Gemma + LLM ziheruka!)
// @description:sa           Google Search (Google Gemma + नूतनतम LLMs द्वारा संचालितम्!) इत्यत्र AI उत्तराणि योजयति ।
// @description:sat          Google Search ar AI jawab khon ojantok (Google Gemma + nutun LLM! sebadha manju)
// @description:sc           Agiungit rispostas de IA a Google Search (motorizadu da Google Gemma + is ultimus LLMs!)
// @description:sd           شامل ڪري ٿو AI جوابن کي Google Search (Google Gemma + جديد LLMs پاران طاقتور!)
// @description:se           Lávdegáhtii AI vástid Google Search (Google Gemma + maŋemus LLM! vuosttas!)
// @description:sg           Nâ tî-kûzâ mái vêdáara AI mbi Google Search (ngâ Google Gemma + LLM tî ngyâ!)
// @description:si           Google Search වෙත AI පිළිතුරු එක් කරයි (Google Gemma + නවතම LLMs මගින් බලගන්වයි!)
// @description:sk           Pridáva odpovede AI do Google Search (poháňané Google Gemma + najnovšími LLM!)
// @description:sl           Dodaja odgovore AI v Google Search (poganja Google Gemma + najnovejši LLM-ji!)
// @description:sm           Faʻaopoopo tali AI ile Google Search (faʻamalosia e Google Gemma + LLM fou!)
// @description:sn           Inowedzera mhinduro dzeAI kuGoogle Search (inofambiswa neGoogle Gemma + LLM dzichangobva!)
// @description:so           Waxay ku dartay jawaabaha AI Google Search (waxaa ku shaqeeya Google Gemma + LLM-yada ugu dambeeyay!)
// @description:sq           Shton përgjigjet e AI në Google Search (mundësuar nga Google Gemma + LLM-të më të fundit!)
// @description:sr           Додаје АИ одговоре у Google Search (покреће најновији ЛЛМ!)
// @description:ss           Iphendvulela izindlela zezilungiselelo ku-Google Search (izenzakalo nge-Google Gemma + ama-LLM akamuva!)
// @description:st           E kopanetse diqoqo tsa AI ka Google Search (ka sebelisoa ke Google Gemma + LLM tsa morao-rao!)
// @description:su           Nambahkeun jawaban AI kana Google Search (dikuatkeun ku Google Gemma + LLM panganyarna!)
// @description:sv           Lägger till AI-svar till Google Search (driven av Google Gemma + de senaste LLM:erna!)
// @description:sw           Inaongeza majibu ya AI kwa Google Search (inaendeshwa na Google Gemma + LLM za hivi karibuni!)
// @description:ta           Google Search க்கு AI பதில்களைச் சேர்க்கிறது (Google Gemma + சமீபத்திய LLMs மூலம் இயக்கப்படுகிறது!)
// @description:te           Google Searchకి AI సమాధానాలను జోడిస్తుంది (Google Gemma + తాజా LLMs ద్వారా ఆధారితం!)
// @description:tg           Ба Google Search ҷавобҳои AI илова мекунад (аз ҷониби Google Gemma + охирин LLMs!)
// @description:th           เพิ่มคำตอบ AI ให้กับ Google Search (ขับเคลื่อนโดย Google Gemma + LLMs ล่าสุด!)
// @description:ti           ናብ Google Search (ብGoogle Gemma + ወቅታዊ LLMs ዝሰርሕ!) ናይ AI መልስታት ይውስኸሉ።
// @description:tk           Google Search-a AI jogaplaryny goşýar (Google Gemma + iň soňky LLMs bilen işleýär!)
// @description:tl           Nagdadagdag ng mga sagot ng AI sa Google Search (pinapatakbo ng Google Gemma + pinakabagong LLMs!)
// @description:tn           O amogela dipotso tsa AI mo Google Search (e a nang le Google Gemma + LLM tsa morago!)
// @description:to           Tambisa mabizo a AI ku Google Search (mukutenga na Google Gemma + LLM fakaʻuhinga!)
// @description:tr           Google Search'ya yapay zeka yanıtları ekler (Google Gemma + en son LLM'ler tarafından desteklenmektedir!)
// @description:ts           Ku engetela tinhlamulo ta AI eka Google Search (leyi fambiwaka hi Google Gemma + LLM ta sweswi!)
// @description:tt           Google Search'ка AI җаваплары өсти (Google Gemma + соңгы LLM белән эшләнгән!)
// @description:tw           Ɔde AI mmuae ka Google Search (a Google Gemma + LLM akyiri na ɛma ahoɔden!) ho.
// @description:ug           Google Search ۋەبسېتكە AI جاۋابلار قوشۇدۇ (Google Gemma + ئەڭ يېڭى LLM تەكشۈرگۈچى بىلەن!)
// @description:uk           Додає відповіді штучного інтелекту в Google Search (на базі Google Gemma + найновіших LLM!)
// @description:ur           Google Search میں AI جوابات شامل کرتا ہے (Google Gemma + جدید ترین LLMs کے ذریعے تقویت یافتہ!)
// @description:uz           Google Search-ga AI javoblarini qo'shadi (Google Gemma + eng so'nggi LLM tomonidan quvvatlanadi!)
// @description:vi           Thêm câu trả lời AI vào Google Search (được cung cấp bởi Google Gemma + các LLM mới nhất!)
// @description:xh           Yongeza iimpendulo ze-AI kwi-Google Search (ixhaswe yi-Google Gemma + ii-LLM zamva nje!)
// @description:yi           לייגט אַי ענטפֿערס צו Google Search (Powered דורך Google Gemma + די לעצטע LLMs!)
// @description:yo           Ṣe afikun awọn idahun AI si Google Search (agbara nipasẹ Google Gemma + awọn LLM tuntun!)
// @description:zh           为 Google Search 添加 AI 答案（由 Google Gemma + 最新 LLMs 提供支持！）
// @description:zh-CN        为 Google Search 添加 AI 答案（由 Google Gemma + 最新 LLMs 提供支持！）
// @description:zh-HK        為 Google Search 添加 AI 答案（由 Google Gemma + 最新 LLMs 提供支援！）
// @description:zh-SG        为 Google Search 添加 AI 答案（由 Google Gemma + 最新 LLMs 提供支持！）
// @description:zh-TW        為 Google Search 添加 AI 答案（由 Google Gemma + 最新 LLMs 提供支援！）
// @description:zu           Yengeza izimpendulo ze-AI ku-Google Search (inikwa amandla yi-Google Gemma + ama-LLM akamuva!)
// @author                   KudoAI
...
(async () => {
    ...
    window.app = {
        ...
        commitHashes: {
            app: '05415f1', // for cached <app|messages>.json
